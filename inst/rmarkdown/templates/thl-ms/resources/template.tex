...
\setmainfont[BoldFont = MinionPro-Bold.otf, 
			...
			BoldItalicFont = MinionPro-BoldIt.otf, Numbers = {Lining}, Ligatures={Common}]{MinionPro-Regular.otf}
\setsansfont[BoldFont = MyriadPro-Bold.otf, 
			ItalicFont = MyriadPro-It.otf,
			BoldItalicFont = MyriadPro-BoldIt.otf]{MyriadPro-Regular.otf}
...
\bigskip
$if(title)$
\title{$title$$if(subtitle)$\\ \large\smallskip $subtitle$$endif$ $if(anonymous)$$else$$if(thanks)$\thanks{$thanks$} $endif$$endif$ \bigskip}
$endif$ 
...
$if(anonymous)$\hfill $else$$for(author)${\sffamily\bfseries $author.name$} \hskip 15pt \emph{\small $author.affiliation$}  $sep$ \par $endfor$ $endif$
